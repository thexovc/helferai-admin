...
import { Users, TrendingUp, AlertCircle, XCircle, Calendar, DollarSign, Repeat, Cpu } from 'lucide-react';
import {
...
const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
        <div style={{ background: '#1a1a2e', padding: '10px 14px', borderRadius: 8, color: '#fff', fontSize: 12 }}>
            ...
            {payload.map((p: any, i: number) => <div key={i} style={{ color: p.color }}>{fmtM(p.value)}</div>)}
        </div>
    );
};
...
export default function StudioDashboard() {
    ...
    React.useEffect(() => {
        const load = async () => {
            const users = await StudioService.getUsers();
            const kpis = await StudioService.getDashboardKPIs();
            const charts = await StudioService.getChartData();
            setData({ users, kpis, charts });
        };
        load();
    }, []);
    ...
    const { users, kpis, charts } = data;
    const total = users.length;
    const active = users.filter((u: any) => u.status === 'Active').length;
    const trial = users.filter((u: any) => u.status === 'Trial').length;
    const expired = users.filter((u: any) => u.status === 'Expired').length;
    const cancelled = users.filter((u: any) => u.status === 'Cancelled').length;
        ...
        <div>
            ...
            <div style={{ padding: 'var(--content-padding)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12, marginBottom: 20 }}>
                    ...
                    <KPICard label="Total APIs" value={kpis.totalApis.value} subtitle="Platform active services" accent="#6c9e4e" icon={<Cpu size={20} />} />
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
                    {[
                        ...
                        { label: 'New This Month', value: 4, icon: <Users size={16} />, color: '#7c5cbf' },
                        { label: 'New Paying', value: 3, icon: <DollarSign size={16} />, color: '#0ea5e9' },
                        { label: 'Conversion Rate', value: '71%', icon: <TrendingUp size={16} />, color: '#7c5cbf' },
                        { label: 'System Health', value: '99.9%', icon: <Cpu size={16} />, color: '#6c9e4e' },
                    ].map(s => (
                    ...
                    <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
                        ...
                        <ResponsiveContainer width="100%" height={220}>
                            <AreaChart data={charts.mrrTrend}>
                                <defs>
                                    <linearGradient id="studioMrrGrad" x1="0" y1="0" x2="0" y2="1">
                                ...
                                <Area type="monotone" dataKey="value" stroke="#7c5cbf" strokeWidth={2.5} fill="url(#studioMrrGrad)" dot={false} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                    <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
                        ...
                        <ResponsiveContainer width="100%" height={220}>
                            <BarChart data={charts.monthlyRevenue} barSize={28}>
                                ...
                                <YAxis tickFormatter={v => `₦${v / 1000}K`} tick={{ fontSize: 10, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
                                <Tooltip content={<CustomTooltip />} />
                                <Bar dataKey="value" fill="#7c5cbf" radius={[6, 6, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                    ...
                    <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <LineChart data={charts.arpuTrend}>
                                ...
                                <Tooltip content={<CustomTooltip />} />
                                <Line type="monotone" dataKey="value" stroke="#6c9e4e" strokeWidth={2.5} dot={{ r: 4, fill: '#6c9e4e' }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                    <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
